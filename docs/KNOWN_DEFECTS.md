...
| **D8** | ✅ 已修（7.0.2 二轮） | 正确答案因"多了一个字"丢掉图通道的分（`用的哪个嵌入模型` 被"备用嵌入模型"挤到 rank2） | 记录侧图节点取自 `entities` 的**中文滑窗碎片**（`{2,6}`），切出什么高度依赖字数对齐，本质是**抽签** |
| **D9** | ✅ 已修（7.0.2 二轮） | 边的 `memory_id` **全是 NULL** → "这条记忆连了哪些图节点"永远查不到 | `add_edges(memory_id=…)` 对 **dict 型边完全忽略**该参数；SQLite 侧 `e.get("memory_id", memory_id)` 在 key 存在且值为 None 时**不回落** |
| **D10** | ✅ 已修（7.0.2 二轮） | **正文里写了日期反而被扣分** —— 正确答案被"没写日期的含糊记忆"挤到 rank3 | 时间通道用正文抽出的 `event_time`，于是"购于 2026-03-15"距今天数被衰减到 ≈0.24，而"备用显卡"（无日期）拿 1.00 |
| **D11** | ✅ 已修（7.0.2 二轮） | 纯改写问句被相关性下限误杀（问"我喜欢什么"得到"没有相关记忆"） | 改写对余弦 0.4900 与无关项 top1 0.5275 **分布重叠** → 单一阈值无法两全；这类问句必须靠**结构化标签通道**兜住 |
...
| D5 | 修前：无关查询 top1 **1.0200** > 相关查询 **0.6922**；修后阈值标定：相关簇 cos min 0.5222 / 无关簇 max 0.4064 |
| D6 | 修前 `dedup` 报出 5 对 0.98~0.986 重复；修后写入侧收口，且**原子不同绝不合并** |
| D7 | 修后 `doctor()["vector_backend"]["breakers"]` 可直接读出剩余熔断时间与分类丢弃数 |
| **端到端质量对照** | **修前 precision@1 = 6/8，且有 1 条无关查询返回 5 条；修后 8/8（冷/预热/无关×4/无关×28 四档全 8/8），无关查询返回 0 条，延迟 p50 0.21 ms** |
| D8 | 修前 8 条候选**全部**标"实体图"（通道失去区分力）；修后 `我用的什么显卡` 的正确答案回到 rank1 |
| D9 | 修前 graph.jsonl / edges 表内 `memory_id` 全为 `null`（实测确认）；修后按记录可取到节点（含 qualifier） |
| D10 | 修前 `购于 2026-03-15` 的时间分 ≈0.24 vs 无日期记录的 1.00（`time_weight=0.20`，足够翻转排序）；修后用记忆自身时效 |
| D11 | 修后 `我喜欢什么样的表达方式` 经标签通道（`喜欢`→`偏好`）命中，`score=1.1694`、`confidence_band=medium` |

> **这 4 条（D8–D11）是被"端到端质量对照"检出的，不是读代码读出来的。**
> 教训：**只有"问题→答案"的标注集 + 同主题干扰项，才能照出排序机制的错。**
> 单看"召回非空、分数不为 0"永远发现不了"正确答案排在第三位"。

### D11 与相关性下限的取舍（为什么必须两条腿走路）
...
两条分布**重叠** —— 任何单一语义阈值都无法既保住前者又挡住后者。结论不是"换个阈值"，
而是**分工**：

- **语义阈值（0.55）负责"宁缺勿滥"** —— 挡住假朋友；
- **结构化标签通道（Path6）负责"结构性可达"** —— 偏好/意图类问句（记忆系统被问得最多、
  且提问天然是改写的一类）由 `喜欢/爱好/习惯/讨厌…` → 标签 `偏好` 的映射兜住。

若只靠收紧阈值，用户问"我喜欢什么"会得到"没有相关记忆"，那是**功能性的破坏**。
这也是为什么 0.45 这个初版阈值必须复标定成 0.55（标定表见 `CHANGELOG.md` 7.0.2 段，
以及 `retrieval.py` 常量旁的注释）。
