...
_ATOM_PATTERNS = [
    ...
    ("ident", re.compile(r"\b(?=[A-Za-z0-9_-]{4,}\b)(?=[A-Za-z0-9_-]*[A-Za-z])"
                         ...
                         re.IGNORECASE), 0),
    # 连字符多段标识（kebab-case）：cn-hangzhou / text-embedding-3-large / ap-southeast
    # 【为什么需要】区域名、模型名、包名大量使用这种形态，而且**改了就对不上**
    # （`cn-hangzhou` 与 `cn-beijing` 是一个字符级的身份差别）。
    # 约束：≥2 段、每段 ≥2 字符、以字母开头 —— 排除日期 `2026-12-31`（数字开头）
    # 与 `-` 连字符散文（"很好-不错" 这类单段不成立）。
    ("kebab", re.compile(r"\b[a-zA-Z][a-zA-Z0-9]*(?:-[a-zA-Z0-9]{2,})+\b",
                         re.IGNORECASE), 0),
